...
dir_processed_pictures = "/home/pi/Desktop/Processed_Pictures_2/"
cropping = True
#lectura CSV
df = pd.read_csv("/home/pi/jv_space/data.csv", index_col = "iter")
...
for index, row in df.iterrows():
    ...
    if row.is_sunlit:
        ...
        Mag_vector = np.sqrt( (row.mag_x**2) + (row.mag_y**2) + (row.mag_z**2))
        df['Mag_val'][index] = Mag_vector
        counter += 1
print(f"Total of images processed {counter}")
df.to_csv(dir_processed_pictures+'processed_data_2.csv')
